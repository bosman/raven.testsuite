...
{
	public class RavenJPathWrapper
	{
		...
		public RavenJPathWrapper(string expression)
		{
		...
		internal RavenJTokenWrapper Evaluate(RavenJTokenWrapper root, bool errorWhenNoMatch)
		{
			var current = root;
			...
			foreach (object part in Parts)
			{
				...
				{
					var o = current as RavenJObjectWrapper;
					if (o != null)
					{
						current = o[propertyName];

						if (current == null && errorWhenNoMatch)
							throw new Exception("Property '{0}' does not exist on RavenJObjectWrapper.".FormatWith(CultureInfo.InvariantCulture, propertyName));
					}
					else
					{
						var array = current as RavenJArrayWrapper;
						if(array != null)
						{
							switch (propertyName)
							{
				...
				else
				{
					...
					var a = current as RavenJArrayWrapper;
					...
					if (a != null)
					{
						...
						{
							if (errorWhenNoMatch)
								throw new IndexOutOfRangeException("Index {0} outside the bounds of RavenJArrayWrapper.".FormatWith(CultureInfo.InvariantCulture, index));
