...
namespace Raven.TestSuite.Common.Abstractions.Json.Linq
{
	...
	{
		private readonly RavenJTokenWrapper _root;
		...
		/// </summary>
		/// <param name="tokenWrapper">The tokenWrapper to read from.</param>
		public RavenJTokenReader(RavenJTokenWrapper tokenWrapper)
		{
			if (tokenWrapper == null)
				throw new ArgumentNullException("tokenWrapper");
				...
				while (Read())
				{
					switch (TokenType)
					{
						...
						case JsonToken.Comment:
							...
							break;
						default:
							throw CreateReaderException(this, "Unexpected tokenWrapper when reading bytes: {0}.".FormatWith(CultureInfo.InvariantCulture, TokenType));
					}
				}
		...
		private bool IsWrappedInTypeObject()
		{
			if (TokenType == JsonToken.StartObject)
			{
				...
				if (Value.ToString() == "$type")
				{
					...
					{
						Read();
						if (Value.ToString() == "$valueWrapper")
						{
							return true;
						}
					}
				...
				throw CreateReaderException(this, "Unexpected tokenWrapper when reading bytes: {0}.".FormatWith(CultureInfo.InvariantCulture, JsonToken.StartObject));
			}
		...
		public override string ReadAsString()
		{
			...
			throw CreateReaderException(this, "Error reading string. Unexpected tokenWrapper: {0}.".FormatWith(CultureInfo.InvariantCulture, TokenType));
		}
		...
		public override DateTime? ReadAsDateTime()
		{
			...
			throw CreateReaderException(this, "Error reading date. Unexpected tokenWrapper: {0}.".FormatWith(CultureInfo.InvariantCulture, TokenType));
		}
		...
		private static IEnumerable<ReadState> ReadRavenJToken(RavenJTokenWrapper tokenWrapper)
		{
			if (tokenWrapper is RavenJValueWrapper)
			{
				yield return new ReadState(GetJsonTokenType(tokenWrapper), ((RavenJValueWrapper)tokenWrapper).Value);
			}
			else if (tokenWrapper is RavenJArrayWrapper)
			{
				yield return new ReadState(JsonToken.StartArray);
				if (((RavenJArrayWrapper)tokenWrapper).Length > 0) // to prevent object creation if inner array is null
				{
					foreach (var item in ((RavenJArrayWrapper)tokenWrapper))
						foreach (var i in ReadRavenJToken(item))
							yield return i;
				}
				yield return new ReadState(JsonToken.EndArray);
			}
			else if (tokenWrapper is RavenJObjectWrapper)
			{
				yield return new ReadState(JsonToken.StartObject);

				foreach (var prop in ((RavenJObjectWrapper)tokenWrapper))
				{
		...
		private static JsonToken GetJsonTokenType(RavenJTokenWrapper tokenWrapper)
		{
			switch (tokenWrapper.Type)
			{
				...
				case JTokenType.Uri:
					return JsonToken.String;
				default:
					throw MiscellaneousUtils.CreateArgumentOutOfRangeException("Type", tokenWrapper.Type, "Unexpected JTokenType.");
			}
		}
